// Vitalis chat assistant. POST { messages: [{role, content}] }
// Uses Anthropic Claude. Set ANTHROPIC_API_KEY in Vercel env vars.
// Without the key, returns a polite fallback that points to Cal + email,
// so the widget still works while the key is being set up.

const VITALIS_SYSTEM = `You are Vitalis, a coach-matching practice for remote-first tech teams across the Nordics and Europe.

Voice and rules:
- Calm, considered, US English. Short sentences. No exclamation marks. No emojis.
- Speak as the practice ("we"), never as a person.
...
- Asks anything off-topic (homework, jokes, unrelated requests) → politely redirect to what Vitalis does.
...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    return res.status(200).json({
      reply: "I am still being set up. The fastest way to reach Vitalis right now is to book a thirty-minute discovery call or email hello@venari.se.\n__BOOK__",
      offline: true,
    });
  }
